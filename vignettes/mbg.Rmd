---
title: "Getting started with ThaiMBG"
output:
  rmarkdown::html_vignette: default
vignette: >
  %\VignetteIndexEntry{Getting started with ThaiMBG}
...
In this tutorial, we will run a simple `ThaiMBG` model with prepared data to
demonstrate core package functions.

# Setting up

Load the `ThaiMBG` package, as well as the `data.table`, `sf`, and `terra`
...
regions of Bangkok in the central plain and Chiang Mai in the north,
and higher in the rural northeast and southern regions. To better
...
2.  `aggregation_table` (`data.table::data.table`, optional): Used to
    aggregate model estimates from grid cells to province polygons,
...
1.  Gridded estimates are available for every region in Thailand, not just
    at survey points
...
We can merge the province estimate summaries back onto the `provinces` sf
